...
`flex.version` is a read-only property that indicates the implemented `flex` version in the current client. For the differences between versions, you can check the [release note](https://github.com/meterio/flex/releases).
...
Returns [`Meter.Block`](#meterblock)

``` javascript
console.log(flex.meter.genesis)
...
``` javascript
const acc = flex.meter.account('0x7567d83b7b8d80addcb281a71d54fc7b3364ffed')
```
...
Returns [`Promise<Meter.Account>`](#meteraccount)
...
Returns `Promise<Meter.Code>`

+ `code` - `string`: Contract code of an account

``` javascript
const acc = flex.meter.account('0x0000000000000000000000000000456E65726779')
...
Given the ABI of contract, we can create a `Meter.Method` object that will be able to simulate a contract call without altering contract state or pack method with arguments to a clause that is ready to sign.

**Parameters**

+ `abi` - `object`: ABI definition of a contract method

Returns `Meter.Method`
...
Returns [`Promise<Meter.VMOutput>`](#metervmoutput)

``` javascript
// Simulate get name from a VIP-180 compatible contract
// Solidity: function name() public pure returns(string)
const nameABI = {}
const nameMethod = flex.meter.account('0x0000000000000000000000000000456E65726779').method(nameABI)
...
const transferABI = {}
const transferMethod = flex.meter.account('0x0000000000000000000000000000456E65726779').method(transferABI)
...
transferMethod
    ...
    .gasPrice('1000000000000000') // 1 MTRG can buy 1000 gas
...
const convertForEnergyMethod = flex.meter.account('0x0000000000000000000000000000456E65726779').method(convertForEnergyABI)
...
Returns [`Meter.Clause`](#meterclause)
...
// Pack a clause that convents 1 MTR to MTRG
// Solidity: function convertForEnergy(uint256 _minReturn) public payable
const convertForEnergyABI = {}
const convertForEnergyMethod = flex.meter.account('0x0000000000000000000000000000456E65726779').method(convertForEnergyABI)
// Set value, leave other arguments unset
...
Given the ABI of a contract, we can create a `Meter.Event` object that will be able to filter contracts events with arguments or pack the arguments to criteria for assembling combined filters.
...
+ `asCriteria`: Pack indexed arguments into criteria for future use, see [`Meter.Filter`](#meterfilter)
+ `filter`: Create an event filter, only accept indexed arguments, see [`Meter.Filter`](#meterfilter)
...
Returns `Meter.Criteria`

``` javascript
// Solidity: event Transfer(address indexed _from, address indexed _to, uint256 _value)
const transferEventABI = {}
const transferEvent = flex.meter.account('0x0000000000000000000000000000456E65726779').event(transferEventABI)
...
// Next you can call the methods of Meter.Filter
```
...
Returns `Meter.BlockVisitor`

+ `revision` - `number|string`: Block number or ID to be visited.

#### Get Block Detail

Returns [`Promise<Meter.Block>`](#meterblock)

``` javascript
const blk=flex.meter.block(0)
...
Returns `Meter.TransactionVisitor`

+ `id` - `number|string`: Block number or ID to be visited

#### Get Transaction Detail

Returns [`Meter.Transaction`](#metertransaction)

``` javascript
const transaction=flex.meter.transaction('0x9daa5b584a98976dfca3d70348b44ba5332f966e187ba84510efb810a0f9f851')
...
Returns [`Meter.Receipt`](#meterreceipt)
...
Filter event and transfer logs on the blockchain. Filter often works with `Flex.Meter.Account`, either creates a filter from an event or packs criteria and then assembles several criteria and sets to a filter. But also there is a way of creating a filter and assembling criteria as per your need then apply it.

**Parameters**

+ `kind` - `'event'|'transfer'`: Kind of filter

Returns `Meter.Filter`

+ `order` - `(order: 'asc'|'desc'): this`: Set the order for filter
+ `range` - `(range: Meter.Filter.Range): this`: Set the range for the filter 
+ `criteria` - `(set: Array<Meter.Filter.Criteria>): this`: Set criteria set for the filter
+ `apply` - `(offset: number, limit: number): Promise<Meter.Filter.Result`: Apply the filter and get the result
...
``` javascript
const filter =  flex.meter.filter('transfer')
...
Filters support two different types of log: `event` and `transfer` so there are two types of `Meter.Filter.Criteria`.

`Meter.Filter.Event.Criteria`:
...
`Meter.Filter.Transfer.Criteria`:
...
+ `set` - `Array<Meter.Filter.Criteria>`: Criteria set for the filter, either array of `Event.Criteria` or an array of `Transfer.Criteria`,items in the criteria array will be combined by `OR` operator to filter the events: e.g. [{"ConA": "A"}, {"ConB": "B", "ConC": "C"}] is '`ConA=A` OR (`ConB=B` AND `ConC=C`)'

``` javascript
const filter=flex.meter.filter('event')
...
Returns [`Promise<Array<Meter.Filter.Result>>`](#meterfilterresult)
...
- `Clauses`  - [`Array<Meter.Clause>`](#meterclause)

Returns [`Promise<Array<Meter.VMOutput>>`](#metervmoutput)

``` javascript
const explainer=flex.meter.explain()
explainer
...
Returns `Meter.Vendor.SigningService`: `Meter.Vendor.TXSigningService` or `Meter.Vendor.CertSigningService`

### Transaction Signing Service

`Meter.Vendor.TXSigningService`:
...
+ `link` - `(url: string): this`: Set the link to reveal transaction-related information, the link will be used for flex to assemble a `callback url` by adding a URL query string param `txid` to the link
...
`TxMessage`:
+ `to`:  Same as [`Meter.Clause.To`](#meterclause)
+ `value`:  Same as [`Meter.Clause.Value`](#meterclause)
+ `data`:  Same as [`Meter.Clause.Data`](#meterclause)
+ `comment` - `string(optional)`: Comment to the clause

Returns `Promise<Meter.Vendor.SigningService.TXResponse>`:
...
signingService
    ...
    .gas(200000) // Set maximum gas
    .link('https://dfinlab.github.io/flex/')
    .comment('Donate 100 MTR and 1000 MTRG to the author of flex')

// Prepare energy transfer clause
const transferABI = {}
const transferMethod = flex.meter.account('0x0000000000000000000000000000456E65726779').method(transferABI)
// Flex author's address and amount in wei
...
signingService.request([
    {
        ...
        data: '0x',
        comment: 'Transfer 100 MTRG'
    },
    {
        comment: 'Transfer 1000 MTR',
        ...energyClause
    }
]).then(result=>{
    console.log(result)
...
`Meter.Vendor.CertSigningService`:
...
Returns  `Promise<Meter.Vendor.SigningService.CertResponse>`:
+ `annex`:
...
``` javascript
const signingService = flex.vendor.sign('cert')
...
+ `meta` - [`Meter.Transaction.Meta`](#metertransaction.meta)
...
+ `outputs` - [`Array<Meter.Receipt.Output>`](#meterreceiptoutput): Clause's corresponding outputs
+ `meta` - [`Meter.Log.Meta`](#meterlogmeta)

### Meter.Receipt.Output

+ `contractAddress` - `string`: Deployed contract address, if the corresponding clause is a contract deployment clause
+ `events` - [`Array<Meter.Log.Event>`](#meterlogevent): Event log objects produced during clause execution
+ `transfers` - [`Array<Meter.Log.Transfer>`](#meterlogtransfer) Transfer log produced during clause execution
...
+ `meta`  - [`Meter.Log.Meta`](#meterlog.meta)
+ `decoded`  - [`Meter.Decoded(optional)`](#meterdecoded): Decoded event log based on the event ABI
...
+ `events` - [`Array<Meter.Log.Event>`](#meterlogevent): Event logs that produced during the execution
+ `transfer` - [`Array<Meter.Log.Transfer`](#meterlogtransfer): Transfer logs that produced during the execution
+ `decoded`  - [`Meter.Decoded(optional)`](#meterdecoded): Decoded returned data based on the method ABI

### Meter.Filter.Result

+ `Meter.Filter.Event.Result` - [`Meter.Log.Event`](#meterlogevent)
+ `Meter.Filter.Transfer.Result` - [`Meter.Log.Transfer`](#meterlogtransfer)
